const students = [ /* <<<<< 101 alunos – copie da lista completa abaixo >>>>> */ ];

async function copy(name, grade) {
    try {
        await navigator.clipboard.writeText(`${name} - ${grade}`);
        alert(`Copiado: ${name} - ${grade}`);
...
function render() {
    ...
    students.forEach(s => {
        ...
        tr.innerHTML = `<td>${s.name}</td><td>${s.grade}</td>
            <td><button class="copy-button" onclick="copy('${s.name}','${s.grade}')">Copiar</button></td>`;
        frag.appendChild(tr);
    });
